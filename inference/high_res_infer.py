...
from src.pipelines.high_res_pipeline import HighResPipeline
...
def run_infer(
    ...
    if infer_type == "single":
        ...
        low_res_image_pil = Image.open(low_res_path).convert("RGB")
        image = pipe(
            [cloth_image_pil],
            [person_image_pil],
            [low_res_image_pil],
        ...
        for cloth_name in tqdm(correspondence_dict.keys(), desc="Processing Images"):
            ...
            low_res_image_pil = Image.open(os.path.join(low_res_root, correspondence_dict[cloth_name] + ".png")).convert("RGB")

            cloth_images.append(cloth_image_pil)
            person_images.append(person_image_pil)
            low_res_images.append(low_res_image_pil)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/high_res_infer.yaml")
    ...
    run_infer(vae, reference_unet, denoising_unet, val_noise_scheduler, cfg.dataset.image_width, cfg.dataset.image_height, 
              weight_dtype, cfg.seed, cfg.dataset.infer_type, cfg.alpha, cfg.beta, cfg.dataset.save_path, data_root=cfg.dataset.data_root, 
              low_res_root=cfg.dataset.low_res_root, batch_size=cfg.dataset.batch_size, cloth_path=cfg.dataset.cloth_path, 
              person_path=cfg.dataset.person_path, low_res_path=cfg.dataset.low_res_path)