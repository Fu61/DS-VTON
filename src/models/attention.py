...
class IdentityWithArgs(nn.Module):
    def forward(self, norm_hidden_states, encoder_hidden_states=None, attention_mask=None):
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if cross_attention_dim is not None or double_self_attention:
            ...
            self.attn2 = IdentityWithArgs()  # is self-attn if encoder_hidden_states is none
        else:
            self.norm2 = None
            self.attn2 = None
...
class BasicTransformerBlock_Denoising(nn.Module):
    def __init__(
        self,
        dim: int,
        num_attention_heads: int,
    ...
    def forward(
        ...
        if self.name:
            ...
            if mode == "write":
                ...
                bank_fea = self_attention_additional_feats[self.name]
