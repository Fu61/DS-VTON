...
from .resnet_denoising import Downsample2D, ResnetBlock2D, Upsample2D
from .transformer_denoising import Transformer2DModel
...
class CrossAttnDownBlock2D(nn.Module):
    ...
    def forward(
        ...
        for i, (resnet, attn) in enumerate(
            zip(self.resnets, self.attentions)
        ):
            # self.gradient_checkpointing = False
...
class DownBlock2D(nn.Module):
    ...
    def forward(self, hidden_states, temb=None, encoder_hidden_states=None):
        ...
        for resnet in self.resnets:
            # print(f"DownBlock2D {self.gradient_checkpointing = }")
            if self.training and self.gradient_checkpointing:
...
class UpBlock2D(nn.Module):
    ...
    def forward(
        ...
        for resnet in self.resnets:
            ...
            # print(f"UpBlock2D {self.gradient_checkpointing = }")
            if self.training and self.gradient_checkpointing:
