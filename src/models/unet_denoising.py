...
from .resnet_denoising import InflatedGroupNorm
from .unet_blocks_denoising import UNetMidBlock2DCrossAttn, get_down_block, get_up_block
...
class UNet2DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        agnostic_image: Optional[torch.Tensor] = None,
        ...
        # pre-process
        # Unpair Warp Mask Changed by Hyman
        # if pose_cond_fea is not None:
        #     sample = sample + pose_cond_fea
        # sample = torch.cat((sample, agnostic_image), dim=1) #comment as now we only need 4 channel input
    ...
    def from_pretrained_checkpoint(
        ...
        if not (config_path.exists() and config_path.is_file()):
            raise RuntimeError(f"{config_path} does not exist or is not a file")
